...
function reset() {
    ...
    // 게임 상태 초기화
    board = Array(ROWS).fill().map(() => Array(COLS).fill(0)); // 게임 보드 초기화
    score = 0; // 점수 초기화
    level = 1; // 레벨 초기화
    lines = 0; // 지운 줄 수 초기화
...
// 블록 떨어뜨리기 함수
function drop() {
    if (!piece.collision(0, 1)) { // 아래로 이동할 수 있는지 확인
        piece.y++; // 조각을 아래로 이동
    } else {
        merge(); // 조각을 보드에 병합
...
// 모바일 터치 컨트롤 마우스 이벤트 리스너 추가 (데스크톱 테스트용)
...
window.addEventListener('orientationchange', () => {
    setTimeout(resizeGame, 200); // 방향 변경 후 약간의 지연 시간을 두고 크기 조정
});
